...
# library(readr)
# boston <- read_csv("boston2013.csv")
...
In this notebook, calculate `x^2`.
...
Now use the `c(...)` command to select rows `7` and `11` (not "7 to 11")
and columns `state` and `cost`.
...
Reread the data into `df` with: (1) column `customer_ID` as `character`;
...
`read.spss` in the package `foreign`.
